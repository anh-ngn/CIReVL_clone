    ...
    def __init__(self, dataset_path: Union[Path, str], split: Literal['train', 'val', 'test'], dress_types: List[str],
        ...
        for dress_type in dress_types:
            if dress_type not in ['dress', 'shirt', 'toptee']:
                raise ValueError(
                    "dress_type should be in ['dress', 'shirt', 'toptee']")
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == 'relative':
            ...
            if self.split == 'val':
                ...
                gt_img_ids = [str(x)
                              for x in self.annotations[index]['gt_img_ids']]
...
class VAWDataset(Dataset):
    ...
    def load_cropped_image(self, img):
        ...
        if self.dilate:
            ...
            left, top = max(0, orig_left - self.dilate *
                            width), max(0, orig_top - self.dilate * height)
            right, bottom = min(im_width, left + (1 + self.dilate) *
                                width), min(im_height, top + (1 + self.dilate) * height)
        else:
