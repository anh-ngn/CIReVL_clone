...
class altCLIP:
    def __init__(self, model_name: str = "google/siglip-base-patch16-256-multilingual", device: Union[str, torch.device] = "cuda"):
        ...
        self.model = AltCLIPModel.from_pretrained("BAAI/AltCLIP-m18")
        self.processor = AltCLIPProcessor.from_pretrained("BAAI/AltCLIP-m18")
    ...
    def encode_text(self, texts: Union[str, List[str]]):
        ...
        text_inputs = self.processor(
            text=texts, return_tensors="pt", padding=True).to(self.device)
        with torch.no_grad():
