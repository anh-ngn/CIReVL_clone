...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default=["img_features", "captions", "mods"],
    ...
    parser.add_argument(
        "--weight-path", type=str, default="", help="Where to store OpenCLIP weights."
    )
    parser.add_argument(
        "--preprocess-type",
        default="targetpad",
    ...
    parser.add_argument(
        "--llm_prompt",
        default="prompts.simple_modifier_prompt",
        type=str,
    ...
    for query_dataset, target_dataset, pairing in zip(
        ...
        termcolor.cprint(
            f"\n------ Evaluating Retrieval Setup: {pairing}",
            color="yellow",
            attrs=["bold"],
        )
        ...
        if result_metrics is not None:
            termcolor.cprint(
                f"Metrics for {args.dataset.upper()} ({args.split})- {pairing}",
                attrs=["bold"],
            )
            ...
            termcolor.cprint(
                f"No explicit metrics available for {args.dataset.upper()} ({args.split}) - {pairing}.",
                attrs=["bold"],
            )
